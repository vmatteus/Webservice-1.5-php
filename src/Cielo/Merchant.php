...
class Merchant
{
    ...
    public function setAffiliationId($id)
    {
        if (!is_numeric($id) || strlen($id) < 1 || strlen($id) > 20) {
            throw new \UnexpectedValueException('O número de afiliação deve ser um número e ter entre 1 e 20 caracteres');
        }
    ...
    public function setAffiliationKey($key)
    {
        if (!is_string($key) || strlen($key) < 1 || strlen($key) > 100) {
            throw new \UnexpectedValueException('A chave de afiliação deve ser uma string e ter entre 1 e 100 caracteres');
        }
